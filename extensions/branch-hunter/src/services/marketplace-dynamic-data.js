;(() => {
  ...
  async function getMarketplaceDynamicData({ listingId, manualFallback = {} }) {
    ...
    const listingType =
      apiCache?.listingType ?? pageListingType ?? manualFallback.listingType ?? null
    ...
    const saleFeePercent =
      saleFeePercentFromApi ?? saleFeePercentFromDerivation ?? manualFallback.saleFeePercent ?? null
    ...
    const reviewRatingSource = typeof apiCache?.reviewRating === "number" ? "api" : null
